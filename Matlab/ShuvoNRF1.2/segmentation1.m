function centroidArray = segmentation1(Im,color) 
redThreshold=20;
greenThreshold=20;
blueThreshold=20;
dustRemove=2;
...
for i=1:imageRow
    for j=1:imageColumn
        if((F(i,j)<=360/360) || (F(i,j)>=350/360))
            F(i,j)=5/360;
        end
    end
end
...
%      Index=((imghsv(:,:,1)<=redHighLimit/360)&(imghsv(:,:,1)>=redLowLimit/360)));
% else
Index=(((imghsv(:,:,1)>=hueLowerLimit/360)...
     &(imghsv(:,:,1)<=hueUpperLimit/360)));
% end
